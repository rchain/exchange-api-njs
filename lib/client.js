...
module.exports = class RClient {
  constructor(host, port, options, isPureJs = true, fastPort = 7070) {
    this.host = host;
    this.port = port;
  ...
  async getTransaction (blockHash) {
    const ops = {
      uri: `http://${this.host}:${this.fastPort}/getTransaction/${blockHash}`
    }
    const resp = await rp(ops)
    return JSON.parse(resp)
  }

  async getTransactionRaw (blockHash) {
